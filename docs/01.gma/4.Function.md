...
&emsp;&emsp;此外，gma 对栅格数据的打开操作重写了 Open 类，主要栅格数据的操作如下：

| 属性 / 方法    | 引用                         | 功能                            |
| :------------- | :--------------------------- | :------------------------------ |
| Bands          | gma.rasp.Open.Bands          | 栅格波段数                      |
| Columns        | gma.rasp.Open.Columns        | 栅格列数                        |
| Rows           | gma.rasp.Open.Rows           | 栅格行数                        |
| ToArray        | gma.rasp.Open.ToArray        | 将栅格数据读取为 Numpy 数组     |
| DataType       | gma.rasp.Open.DataType       | 栅格数据的数据类型              |
| Driver         | gma.rasp.Open.Driver         | 栅格数据驱动                    |
| GeoTransform   | gma.rasp.Open.GeoTransform   | 栅格仿射变换                    |
| GetBand        | gma.rasp.Open.GetBand        | 读取某一波段为 gdal.Band        |
| GetBandToArray | gma.rasp.Open.GetBandToArray | 将某一波段读取为 Numpy 数组     |
| GetGDALDataset | gma.rasp.Open.GetGDALDataset | 读取栅格数据为 gdal.GDALDataset |
| NoData         | gma.rasp.Open.NoData         | 栅格无效值                      |
| Projection     | gma.rasp.Open.Projection     | 栅格坐标系                      |
