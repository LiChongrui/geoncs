...
&emsp;　本页对 233 个 其他类型的地理坐标参考系（Geographic Coordinate Reference System，GEOGCRS）进行汇总，由于其与原有的地理坐标系统有所不同，故分开统计。

<!-- more -->

## 简介

&emsp;　地理坐标参考系具有地理坐标系的特征。例如，在利用 osr （osgeo 的模块）进行识别时，其与地理坐标系仍能识别为一类。但其具有轴相关参数、适用范围边界等一些特殊的参数。

## 轴

&emsp;　地理坐标参考系有大地经度、大地维度和椭球体高度三个重要参数，本页统计的所有参考系*（除了 EPSG:4327（已废弃），其不具有大地经度）*这三个参数相同，具体参数值及其对照如下：
